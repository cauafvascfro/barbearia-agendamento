...
export default async function DashboardPage() {
  ...
  return <div className="stack-lg">
    <header className="split"><div><div className="wrap"><p className="eyebrow">{agora.setLocale('pt-BR').toFormat("cccc, dd 'de' LLLL")}</p><span className={`badge ${instalacaoPronta && config?.agenda_publica_ativa!==false?'badge-green':'badge-gray'}`}>{!instalacaoPronta?'Agenda em preparação':config?.agenda_publica_ativa===false?'Agenda pública pausada':'Agenda pública online'}</span></div><h1 className="page-title">{config?.nome_barbearia||'Dashboard'}</h1></div><div className="wrap"><Link className="btn" href="/admin/clientes">Clientes</Link><Link className="btn btn-primary" href={`/admin/agenda?data=${agora.toISODate()}`}>+ Novo agendamento</Link></div></header>
