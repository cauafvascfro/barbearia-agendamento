...
export default async function DashboardPage() {
  ...
  const { data: config } = await supabase.from('configuracoes').select('nome_barbearia,timezone,agenda_publica_ativa').limit(1).single()
  ...
  const [{data:hoje},{data:mes},{count:clientesTotal}]=await Promise.all([
