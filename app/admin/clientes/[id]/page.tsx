...
export default async function ClientePage({ params }: Props) {
  ...
  return <div className="stack-lg">
    ...
    <header className="split"><div><h1 className="page-title">{cliente.nome}</h1><p className="muted">{formatarTelefone(cliente.telefone)}</p></div><div className="wrap"><Link className="btn btn-primary" href={`/admin/agenda?data=${DateTime.now().setZone(timezone).toISODate()}&cliente=${cliente.id}`}>+ Novo agendamento</Link>{cliente.telefone&&<a className="btn" href={`tel:${String(cliente.telefone).replace(/\\D/g,'')}`}>Ligar</a>}</div></header>
