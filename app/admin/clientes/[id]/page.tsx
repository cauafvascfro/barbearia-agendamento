...
export default async function ClientePage({ params }: Props) {
  ...
  return <div className="stack-lg">
    ...
    <header className="split"><div><h1 className="page-title">{cliente.nome}</h1><p className="muted">{formatarTelefone(cliente.telefone)}</p></div>{cliente.telefone&&<a className="btn btn-primary" href={`tel:${String(cliente.telefone).replace(/\\D/g,'')}`}>Ligar para cliente</a>}</header>
