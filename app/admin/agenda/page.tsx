...
export default async function AgendaPage({ searchParams }: Props) {
  ...
  const [{ data: agendamentos }, { data: bloqueios }, { data: servicos }, { data: expediente }, { data: aberturasExtras }] = await Promise.all([
...
function mensagemErro(erro:string) { const m:Record<string,string>={dados:'Preencha os dados obrigatórios.',ocupado:'Este horário já está ocupado.',bloqueado:'Este horário está bloqueado.',expediente:'O horário está fora do expediente.',antecedencia:'O horário está dentro da antecedência mínima configurada.',bloqueio:'Período de bloqueio inválido.',bloqueio_conflito:'Existe um atendimento neste período.',abertura:'Horário especial inválido.',status:'Status inválido.',banco:'Não foi possível concluir a operação.'}; return m[erro]||'Ocorreu um erro.' }
