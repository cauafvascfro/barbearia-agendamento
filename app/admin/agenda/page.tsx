    ...
    <div className="stack-lg">
      ...
      <div className="admin-grid">
        <section className="stack">
          ...
          {ativosVisiveis.map((a) => <AppointmentCard key={a.id} a={a} timezone={timezone} data={dataSelecionada} reagendarData={hoje} destaque={proximo?.id === a.id}/>)}
          ...
          {finalizadosVisiveis.map((a) => <AppointmentCard key={a.id} a={a} timezone={timezone} data={dataSelecionada} reagendarData={hoje}/>)}
        </section>
...
function AppointmentCard({a,timezone,data,reagendarData,destaque=false}:{a:any;timezone:string;data:string;reagendarData:string;destaque?:boolean}) { const inicio=DateTime.fromISO(a.inicio).setZone(timezone); const fim=DateTime.fromISO(a.fim).setZone(timezone); const cliente=Array.isArray(a.cliente)?a.cliente[0]:a.cliente; return <article className={`card appointment appointment-${String(a.status).toLowerCase()} ${destaque?'appointment-next':''}`}><div><div className="appointment-time">{inicio.toFormat('HH:mm')}</div><div className="muted small">até {fim.toFormat('HH:mm')}</div></div><div><div className="wrap"><strong>{cliente?.nome||'Cliente'}</strong>{destaque&&<span className="badge badge-yellow">Próximo</span>}<Status status={a.status}/></div><p>{a.nome_servico}</p><p className="muted small">{formatarTelefone(cliente?.telefone)} · {formatarMoeda(a.preco)} · {a.origem==='SITE'?'Online':'Manual'}</p><div className="wrap appointment-links">{cliente?.id&&<Link className="appointment-client-link small" href={`/admin/clientes/${cliente.id}`}>Ver cliente →</Link>}{cliente?.id&&<Link className="appointment-client-link small" href={`/admin/agenda?data=${reagendarData}&cliente=${cliente.id}`}>Agendar novamente</Link>}</div>{a.observacoes&&<p className="small">{a.observacoes}</p>}</div>{a.status==='CONFIRMADO'&&<div className="appointment-actions"><details className="appointment-reschedule"><summary className="btn">Remarcar</summary><form action={remarcarAgendamentoAdmin} className="stack"><input type="hidden" name="id" value={a.id}/><input type="hidden" name="data_origem" value={data}/><Campo label="Nova data" name="nova_data" type="date" required/><Campo label="Novo horário" name="nova_hora" type="time" required/><button className="btn btn-primary">Confirmar remarcação</button></form></details><StatusButton id={a.id} data={data} status="CONCLUIDO" primary>✓ Concluir</StatusButton><StatusButton id={a.id} data={data} status="NAO_COMPARECEU">Faltou</StatusButton><StatusButton id={a.id} data={data} status="CANCELADO" danger>Cancelar</StatusButton></div>}</article> }
